# -*- coding: utf-8 -*-
"""
Gemini (Gemini CLI OAuth) quota fetcher.

IMPORTANT: Google discontinued Gemini CLI OAuth for individual / AI Pro / Ultra
accounts on 2026-06-18. Free-tier individual accounts now hit `UNSUPPORTED_CLIENT`
on the quota endpoint. Standard/Enterprise and Workspace/Edu accounts still work.
We surface that as an explicit unavailable reason so the footer stays honest.

Data source (CodexBar's gemini.md):
  POST https://cloudcode-pa.googleapis.com/v1internal:retrieveUserQuota
  Authorization: Bearer <access_token from ~/.gemini/oauth_creds.json>
  Body: {"project": "<quota project id>", "metadata": {...}}  (project optional)

Response carries per-model quota buckets:
  { "quota": [ { "modelId": "...", "remainingFraction": 0.42, "resetTime": "ISO" } ] }
We take the LOWEST remainingFraction across models as the effective left%.

Token refresh uses Google OAuth (client_id/secret from the Gemini CLI install or
env overrides GEMINI_OAUTH_CLIENT_ID / GEMINI_OAUTH_CLIENT_SECRET).
"""
...
def _extract_client_from_js() -> tuple[Optional[str], Optional[str]]:
    """Best-effort: pull OAUTH_CLIENT_ID/SECRET from the Gemini CLI oauth2.js."""
    import subprocess
...
def _parse_quota(data: dict) -> Optional[QuotaResult]:
    ...
    for b in buckets:
        ...
        if best is None or (w.used_percent or 0) > (best.used_percent or 0):
            # lowest remaining -> highest used wins (worst model)
