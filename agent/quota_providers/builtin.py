"""Built-in provider quota fetchers that reuse ``agent.account_usage``.

OpenAI Codex, Anthropic, Nous, and OpenRouter already expose quota via
``fetch_account_usage`` (AccountUsageSnapshot with per-window used_percent +
reset).  We adapt those snapshots into the QuotaResult shape and register them so
the cache builder can treat them uniformly with the Grok (and future) fetchers.
"""
