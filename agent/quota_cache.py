...
(session / weekly / monthly) with its remaining % and reset time.  Showing live
...
provider), plus the footer has no live agent / credentials in scope.  Instead:
  ...
  * provider fetchers live in ``agent.quota_providers`` (a pluggable registry);
...
from agent.quota_providers import PROVIDER_FETCHERS, QuotaResult
...
def refresh_quota_cache(*, timeout: float = 12.0) -> dict[str, Any]:
    ...
    Returns the cache dict that was written (handy for tests / dry runs).
    """
